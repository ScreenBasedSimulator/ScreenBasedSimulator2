...
public class UnitConverter
{
  ...
  static
  {
    ...
    try
    {
      Files.copy(new File(UCEDefsPath).toPath(), new File(UCEDefsHomePath).toPath(), REPLACE_EXISTING);
    }
    catch (IOException e)
    {
    }

    List<String> files = FileUtils.findFiles(System.getProperty("user.dir"), "CommonDataModelJNI", true);
    if(files.size()!=1)// Attempt to load and hope for the best...  
      System.loadLibrary("CommonDataModelJNI");
    else
      System.load(files.get(0));   

    try
    {
      Files.delete(new File(UCEDefsHomePath).toPath());
    }
    catch (Exception e)
    {
    }
  ...
  private static native double nativeInitialize(String workingDirectory);
