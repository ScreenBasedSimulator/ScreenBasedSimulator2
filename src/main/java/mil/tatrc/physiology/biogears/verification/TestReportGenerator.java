...
{  
  ...
  { 
    ...
    {
      if (args.length >= 2)
    	  generator.setCommitHash(args[1]);
      
      if (args.length >= 3)
      {
          generator.setEnvironment(args[2]);
      }
  ...
  {    
    ...
    for(String key : keySet)
    {
      ...
      if (key.equals("percentDifference") ||
          key.equals("timeStart"))
      {
        continue;
      }
      ...
      else
      {
        ...
        boolean knownFailure = key.charAt(0) == '*';
        addToSummary(this, args[0], knownFailure);
      }
    }
    // Done processing
    if(mode==null)
  ...
  public boolean addToSummary(TestReport summary, String testReportFile, boolean knownFailure)
  {
    if (knownFailure)
    {
        summary.addKnownSuiteFailure(testReportFile);
        return true;
    }
