...
import mil.tatrc.physiology.biogears.verification.ScenarioResult;
...
public abstract class ValdiationTool
{
  ...
  protected void CustomMarkdown(String sheetName, String destinationDirectory, Object o){};
  
  protected StringBuilder html = new StringBuilder();
  
  protected enum DataType { Mean, MeanPerWeight, WaveformMin, WaveformMinPerWeight, WaveformMax, WaveformMaxPerWeight, Enum, SystemMean, SystemMin, SystemMax }
  
  protected SEPatient                       patient;
  protected Map<String,List<Double>>        resultData;
      ...
      for(int i=0; i<xlWBook.getNumberOfSheets(); i++)
      {
        ...
        for(String resultsName : sheetFiles)
        {
          ...
          {          
            ...
            ScenarioResult results = new ScenarioResult("./Scenarios/Validation/"+resultsName);
            resultData=results.readAll();
          ...
          catch(Exception ex)
          {
            ...
            if(sheetName.equals("Nervous"))
              vRow.error=warning+"System not currently supported"+endSpan;
            else
          ...
          if(TABLE_TYPE.equals("Patient"))
          {
            ...
            String pFile = resultsName.substring(8/*length of "Patient-"*/,resultsName.indexOf("Validation"));
            patient.load((PatientData)CDMSerializer.readFile("./patients/"+pFile+".xml"));
          }
          ...
          try
          {
            ...
            for (int r = 0; r < rows; r++) 
            {
              ...
              if (row == null) 
                continue;
              int cells = 10;//row.getPhysicalNumberOfCells();
          ...
          List<ValidationRow> all=null;
          if(patient!=null)
            all = new ArrayList<ValidationRow>();          
          ...
          if(all!=null)
          {
            WriteHTML(all,patient.getName());
            WriteDoxyTable(all,patient.getName(),destinationDirectory);
            CustomMarkdown(patient.getName(),destinationDirectory,patient);            
          }
        }
      }
      xlWBook.close();
  ...
  protected boolean buildExpectedHeader(ValidationRow vRow)
  {
    ...
    if(rVals.length==1)
    {
      ...
      else if(vRow.dType==DataType.SystemMean || vRow.dType==DataType.SystemMax || vRow.dType==DataType.SystemMin)
      {
        String n = vRow.name;
        vRow.name = vRow.refValues;
        vRow.refValues = n;
        vRow.header = vRow.name;        
        if(vRow.dType == DataType.SystemMax)
          vRow.header += " Maximum";
        if(vRow.dType == DataType.SystemMin)
          vRow.header += " Minimum";
        if(vRow.unit!=null && !vRow.unit.isEmpty())
          vRow.header += "("+vRow.unit+")";
        vRow.refValue = getResults("Patient"+vRow.refValues, vRow.unit).get(0); 
        vRow.expected = vRow.refValue+" [Baseline]";
      }
  ...
  {
    //if(vRow.name.indexOf("FunctionalResidualCapacity")>-1)
    //  Log.info("Break");
    if(!getResult(vRow))
    {
      return false;
    ...
    switch(vRow.dType)
    {
      ...
      case WaveformMinPerWeight:
      {
        ...
        vRow.result = WaveformUtils.getMin(resultPerWeight);
        vRow.engine = "Minimum of " + String.format("%."+3+"g", vRow.result); //StringUtils.toString(vRow.mean,3);
        break;
      }
      case WaveformMax:
    ...
    {// What is the % difference between the value and the test value      
      if(vRow.dType == DataType.SystemMax && vRow.result <= vRow.refValue)
      {
        vRow.resultError = 0;
        vRow.error  = success+"Under Max"+endSpan;
        return true;
      }
      if(vRow.dType == DataType.SystemMin && vRow.result >= vRow.refValue)
      {
        vRow.resultError = 0;
  ...
  {
    //if(vRow.name.equals("ExpiratoryReserveVolume"))
    //  Log.info("Here");
    if(vRow.dType == DataType.MeanPerWeight || vRow.dType == DataType.WaveformMinPerWeight || vRow.dType == DataType.WaveformMaxPerWeight)
  ...
  protected List<Double> getResults(String name, String unit)
  {
    ...
    if(unit!=null && !unit.isEmpty())
      header = name+"("+unit+")";
    List<Double> results = resultData.get(header.replace(' ','_'));
    if(results==null)
    {// Maybe the result is in a different unit than our reference value
      String newUnit=null;
      String[] split;
      for(String result : resultData.keySet())
      {
        ...
        if(split.length==2&&split[0].equals(name))
        {
          ...
          {
            double newValue = UnitConverter.convert(results.get(i), newUnit, unit);
