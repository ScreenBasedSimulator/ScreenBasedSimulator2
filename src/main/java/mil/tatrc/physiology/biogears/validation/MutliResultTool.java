...
import mil.tatrc.physiology.biogears.verification.ScenarioPlotTool;
...
public class MutliResultTool
{
  ...
  protected String processResultsFile(String resultsDir, String resultsFile, Map<String,List<ResultSummary>> resultSummaries, Map<String,List<ResultSummary>> baselineSummaries, ScenarioPlotTool plotter) throws FileNotFoundException
  {
    String xValue = null;
    String xUnit  = null;
    ...
    {            
      ...
      ScenarioResult sceResults = new ScenarioResult(resultsDir+resultsFile);
      ...
      Map<String,List<Double>> resultData=sceResults.readAll();     
      if(plotter!=null)
      {
        ...
        for(String header : resultData.keySet())
        {
          if(header.equals("Time(s)"))
            continue;
          plotter.createGraph(resultsDir+xValue+xUnit+"/", Color.white, header + "_vs_Time","Time",header,plotter.createTruncatedXYSeries(header,resultData.get("Time(s)"),resultData.get(header)));
        }
      }
