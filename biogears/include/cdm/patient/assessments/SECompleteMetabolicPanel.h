...
CDM_BIND_DECL(CompleteMetabolicPanelData)

class DLL_DECL SECompleteMetabolicPanel : public SEPatientAssessment
{
public:

	SECompleteMetabolicPanel(Logger* logger);
	virtual ~SECompleteMetabolicPanel();

	virtual void Reset(); //reset values
	virtual void Clear(); //clear memory

	bool Load(const CDM::CompleteMetabolicPanelData& in);
	std::unique_ptr<CDM::CompleteMetabolicPanelData> Unload();
protected:
	void Unload(CDM::CompleteMetabolicPanelData& data);
public:
	bool HasAlbumin();
