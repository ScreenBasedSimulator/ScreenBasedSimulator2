...
**  COPYRIGHT (C) 2004, APPLIED RESEARCH ASSOCIATES, INC., ALL RIGHTS RESERVED.
**
*****************************************************************************/
...
public:
	...
	typedef hash_map<char,unsigned int> PrefixMap;
	typedef hash_map<std::string,unsigned int> UnitSymbolMap;
	typedef hash_map<std::string,unsigned int> QuantityNameMap;
	typedef hash_map<CQuantityConversionKey, unsigned int> QuantityConversionMap;
	typedef hash_map<CUnitDimension, unsigned int> DimensionToQuantityMap;
	...
	void SetWorkingDirectory(std::string wrkDir)
	{
		m_wrkDir = wrkDir;
	}
